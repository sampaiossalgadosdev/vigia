/**
 * Arquivo: contaPagar.validator.js
 * Responsabilidade: Schema de validação de criação de ContaPagar.
...
const criar = [
  ...
  body('fornecedorId').optional({ values: 'falsy' }).isUUID().withMessage('fornecedorId inválido'),
  body('observacao').optional({ values: 'falsy' }).isString().trim().isLength({ max: 500 }),
  validar,
];

const baixa = [
  body('dataPagamento').optional({ values: 'falsy' }).isISO8601().withMessage('Data de pagamento inválida'),
  body('formaPagamento').optional({ values: 'falsy' }).isString().trim().isLength({ max: 50 }),
  validar,
];
